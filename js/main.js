;(function (global) {
    ...
    (function() {
        ...
        if(parameter){
            ...
            for (let i = 0; i < parameters.length; i++) {
                ...
                parameterObj[pair[0]] = pair[1];
            }
            Main.pathParameter = parameterObj;
        }
     ...
     * @param obj
     */
    Main.message = function(obj){
        if(obj.constructor === String){
            obj = {message: obj};
        }
        if(obj.constructor === Object){
            let dom = document.createElement('div');
            dom.style.cssText = 'position: fixed;z-index: 200000;padding: 15px; left: 50%; top: 50%;background-color: rgba(0, 0, 0, 0.4);color: white;transform: translate(-50%, -50%);'
            //文字颜色
            if(obj.color){
            ...
            //样式文本
            if(obj.cssText){
                dom.style.cssText += obj.cssText;
            }
            // dom.style.cssText = defaultCssText;
            dom.innerHTML = obj.message;
    ...
    Main.loading = function () {
        let dom = document.createElement('div');
        dom.style.cssText = 'position: fixed;z-index: 99999999999; background-color: rgba(0, 0, 0, 0); top:0; left:0; width: 100%; height: 100%;display: flex;align-items: center;justify-content: center;'
        let chDom = document.createElement('div');
        chDom.style.height = '24px';
        chDom.style.width = '60px';
        let url = Main.path.pathDispose('@/img/loading/loading.gif');
        chDom.style.background = "url("+ url +") no-repeat"
...
//===========================================================================
            ...
            mergeConfig: function (obj) {
                ...
                //设置框架全局提示框
                obj.$message = Main.message;
                //设置框架全局等待框
                obj.$loading = Main.loading;
                //设置框架全局关闭框
                obj.$close = Main.close;
            },
                ...
                this.parseAttrMJS(attr, function (obj) {
                    ...
                    if(parseObj){
                        ...
                        for(let c of parseObj.nameList){
                            if(c == '$dom'){
                                parameterAdd.push(childComObj.elDom);
                            }else{
                ...
                this.parseAttrMJS(attr, function (obj) {
                    ...
                    if(parseObj){
                        ...
                        for(let c of parseObj.nameList){
                            if(c != '$dom' && c != '$event'){
                                let v = this.parseFrameString(comObj, c);
                                let parameterName = `parameterName${++i}`;
                                parameterObj[parameterName] = v;
                                csStr += 'parameterObj.' + parameterName + ',';
                            }else{
                                csStr += c + ',';
                            }
                        }
                        if(csStr){
                            ...
                            funRun = new Function('$event', 'obj', '$dom', 'parameterObj', csStr);
                        }
                    }
                    ...
                    if (comObj[obj.value]) {
                        if(funRun){
                            f = function (event) {
                                funRun(event, comObj, dom, parameterObj);
                            }
                        }else{
                            f = function (event) {
